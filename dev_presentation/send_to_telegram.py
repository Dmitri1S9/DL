...
from lib.telegram import get_bot_token, get_chat_id, send_telegram  # noqa: E402
...
DOCS = [
    ...
    (PDF / "dl-presentation-brief.pdf", "📘 Master brief — everything behind the slides"),
]
...
def send_document(path: Path, caption: str = "", chat_id: str = "") -> bool:
    ...
    if os.environ.get("DRY_RUN", "false").lower() == "true":
        print(f"[DRY RUN] would send {path.name} ({path.stat().st_size // 1024} KB) — {caption}")
        return True
