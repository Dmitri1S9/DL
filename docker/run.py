...
Usage:
    python run.py                        # process every wav in /input
    python run.py --shard 0 2            # first half (one of two machines)
    python run.py --shard 1 2            # second half
"""
...
def setup_model():
    ...
    os.environ['weight_root'] = MODELS_DIR  # noqa: SIM112
    os.environ['index_root'] = MODELS_DIR  # noqa: SIM112
    os.environ['outside_index_root'] = MODELS_DIR  # noqa: SIM112
...
def process_file(vc, wav_path: str, out_path: str) -> None:
    result = convert_b1(vc, wav_path, INDEX_PATH)
    ...
    while True:
        ...
        if not todo:
            if not args.watch:
                ...
                return
            idle_rounds += 1
            if idle_rounds >= 4:  # 4 x 15s = 60s with no new files
