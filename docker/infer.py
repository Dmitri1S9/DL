...
from rvc_common import convert_b1

# RVC source lives at /rvc; its modules are imported inside main().
    ...
    if pth_files:
        ...
        with zipfile.ZipFile(zip_src) as zf:
            ...
            if not extracted_pth:
                print('ERROR: no .pth inside zip', file=sys.stderr)
                sys.exit(1)
        ...
        print('ERROR: no .pth or .zip in repo', file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    ...
    for wav in wavs:
        ...
        wav_opt = convert_b1(vc, str(wav), str(INDEX_PATH), f0_method='rmvpe')
