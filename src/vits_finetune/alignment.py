...
def neg_cross_entropy(
    ...
    const_term = torch.sum(-0.5 * math.log(2 * math.pi) - prior_log_stddev, dim=1, keepdim=True)
    ...
    z_sq_term = -0.5 * torch.matmul(precision.transpose(1, 2), z_p**2)  # (B, T_text, T_frames)
...
def maximum_path(neg_cent: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    direction = torch.zeros((batch_size, num_text, num_frames), dtype=torch.bool, device=device)
    ...
    for frame in range(num_frames):
        ...
        running = torch.where(
            reachable, best_prev + value[:, :, frame], torch.full_like(running, _NEG_INF)
        )
