...
class Trainer:
    def __init__(self, config, model_config, disc_config, args):
        ...
        self.dataset = VitsFinetuneDataset(
            config, self.tokenizer, split=config.train_split
        )
        ...
        self.optimizer_G = torch.optim.AdamW(
            self.model_G.parameters(), lr=config.learning_rate
        )
        # Discriminator
    ...
    def __pretrain_check(func):
        def wrapper(self):
            if self.args.resume:
                ...
                d_resume = self.args.resume.parent / self.args.resume.name.replace(
                    '_G.pt', '_D.pt'
                )
                if d_resume.exists():
                    load_checkpoint(
                ...
                logger.info(
                    f'Resumed from {self.args.resume} at epoch {self.start_epoch}, step {self.global_step}'
                )
            return func(self)
    ...
    def __back_step_dec(func):
        def wrapper(self) -> None:
            for epoch in range(self.start_epoch, self.config.num_epochs):
                for batch in self.dataloader:
                    ...
                    if self.global_step % self.config.log_every == 0:
                        logger.info(
                            ...
                            + ' | '.join(
                                f'{k}: {v:.4f}' for k, v in self.loss_dict.items()
                            )
                        )
                    if self.global_step % self.config.checkpoint_every == 0:
                        gp = (
                            self.config.checkpoint_dir / f'step_{self.global_step}_G.pt'
                        )
                        dp = (
                            self.config.checkpoint_dir / f'step_{self.global_step}_D.pt'
                        )
                        save_checkpoint(
                            gp, self.model_G, self.optimizer_G, self.global_step, epoch
                        )
                        save_checkpoint(
                            dp,
                            self.discriminator,
                ...
                save_checkpoint(
                    gp, self.model_G, self.optimizer_G, self.global_step, epoch + 1
                )
                save_checkpoint(
                    dp,
                    self.discriminator,
    ...
    def __log_parts(func):
        def wrapper(self, *args, **kwargs):
            ...
            if self.global_step % self.config.log_every == 0:
                logger.info(
                    '  parts | ' + ' | '.join(f'{k}: {v:.4f}' for k, v in parts.items())
                )
            return result
    ...
    def __generator_step(self, outputs, real_wave, fake_wave):
        ...
        with self._amp():
            ...
            g_loss, _ = self.__calculate_loss_g(
                outputs, fake_outs, real_fmaps, fake_fmaps
            )
...
def main() -> None:
    ...
    logging.getLogger('phonemizer').setLevel(
        logging.ERROR
    )  # silence "words count mismatch" spam
