...
_MAX_SAMPLES = int(11.0 * config.SAMPLE_RATE)


def pad_or_skip(audio: np.ndarray) -> np.ndarray | None:
    """Pad short audio with silence. Return None if longer than 11s — caller skips."""
    if len(audio) > _MAX_SAMPLES:
        return None
    if len(audio) < _MAX_SAMPLES:
        return np.concatenate([audio, np.zeros(_MAX_SAMPLES - len(audio), dtype=np.float32)])
    return audio
...
class Magic:
    ...
    def augment_and_save(self, out_dir: Path = config.AUDIO_DIR / "augmented") -> Path:
        ...
        for i, sample in enumerate(self.ds):
            ...
            if result is None:
                logger.warning(f"[{i}] skipped — longer than 11s")
                skipped += 1
                continue

            accented_text, b1_audio = result
            item = self._save_sample(b1_audio, accented_text, f"sample_{i:02d}", out_dir)
            items.append(item)
        ...
        logger.success(f"Done: {len(items)} saved, {skipped} skipped. Manifest -> {manifest_path}")
        return manifest_path


def demo_b1(out_path: Path = config.AUDIO_DIR / "b1_demo.wav") -> None:
    """Синтезирует фразу через TTS модель и применяет B1 эффект."""
...
def demo_pipeline(out_dir: Path = config.AUDIO_DIR / "pipeline_demo") -> None:
    """Показывает весь пайплайн на 1 реальном сэмпле чистой речи.

    Скачивает 1 сэмпл из LibriSpeech test-clean (отдельные flac, не tar.bz2)
    -> прогоняет через Magic -> сохраняет wav + manifest.jsonl.
    """
    out_dir.mkdir(parents=True, exist_ok=True)

    logger.info("[1/4] Скачиваем 1 чистый сэмпл из LibriSpeech test-clean...")
    ds = load_dataset(
    ...
    logger.success(f"      step1_original.wav  ({len(audio)/config.SAMPLE_RATE:.1f}s)")
    logger.info(f"      text: '{original_text[:80]}'")

    logger.info("[2/4] Применяем аугментацию (акцент + B1 дроид)...")
    magic = Magic.__new__(Magic)
    result = magic._augment_sample(original_text, audio)

    if result is None:
        logger.warning("Сэмпл длиннее 11с — пропущен")
        return

    accented_text, b1_audio = result
    sf.write(str(out_dir / "step2_b1_droid.wav"), b1_audio, config.SAMPLE_RATE)
    logger.success(f"      step2_b1_droid.wav  ({len(b1_audio)/config.SAMPLE_RATE:.1f}s)")
    logger.info(f"      accented: '{accented_text[:80]}'")

    logger.info("[3/4] Сохраняем манифест (тренировочная пара text+audio)...")
    item = magic._save_sample(b1_audio, accented_text, "demo_sample", out_dir)
    write_manifest([item], out_dir / "manifest.jsonl")

    logger.success("=== Результат ===")
    logger.success(f"  step1_original.wav  — исходная чистая речь")
    logger.success(f"  step2_b1_droid.wav  — то что пойдёт как таргет на обучение")
