...
def evaluate_dataset(
    ...
    start = max(0, total - n)
    logger.info(
        f'Shard has {total} clips; scoring the last {total - start} (the ceiling set).'
    )
    ...
    hypotheses: list[str] = []
    references: list[str] = []
    for i, (clip, text) in enumerate(zip(audio_col, texts, strict=True)):
        wav = _decode_clip(clip)
        hypotheses.append(transcribe(wav, whisper_size))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='WER/CER of the real B1 recordings (ceiling).'
    )
    parser.add_argument(
        '--n', type=int, default=100, help='How many held-out clips to score.'
    )
    parser.add_argument('--shard', default=LAST_SHARD)
    parser.add_argument('--whisper', default='base')
    parser.add_argument(
        '--out', type=Path, default=config.ROOT / 'logs' / 'eval_ceiling.json'
    )
